...
- **Fixed hook chain ordering.** The internal `hookChain` runs hooks in a deliberate order — PII → Audit → custom — documented at `hook.go:91-103` and enforced by the append order in `buildHooks` (`hook.go:196`). The ordering matters: PII masking must finish before `AuditHook` hashes the entry. Any new hook must slot in at the right position, not be appended unconditionally.
