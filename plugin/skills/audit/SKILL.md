...
- `FileStore` locks `path + ".lock"` with flock from its first write for the life of the process: a second process on the same store fails in `New` with `ErrChainStoreLocked` (no lock on Windows, Solaris and AIX)
