---
name: pii-masking
description: "This skill should be used when the user asks about PII masking in axio — PIIMasker, PIIHook, MaskString, MaskStringWithCounts, DefaultPIIConfig, PIIPattern, CustomPII, PatternCPF, PatternCNPJ, PatternCreditCard, PatternEmail, PatternPhone, sensitive fields, DefaultSensitiveFields, WithPII option, WithPIIDisabled, field redaction, or LGPD/GDPR compliance logging. Trigger phrases include \"PII\", \"masking\", \"PIIMasker\", \"PIIHook\", \"MaskString\", \"DefaultPIIConfig\", \"PatternCPF\", \"PatternCNPJ\", \"PatternEmail\", \"PatternPhone\", \"PatternCreditCard\", \"CustomPII\", \"sensitive fields\", \"REDACTED\", \"WithPII\", \"LGPD\", \"mask personal data\", \"credit card masking\", \"CPF masking\"."
---

# PII Masking

Masking is on by default in every Logger and Event: axio detects and masks sensitive personal data in log messages and structured fields. `WithPII` configures it, and `WithPIIDisabled()` or `piiDisabled: true` turns it off.
...
The default PIIHook runs FIRST, and auditing hashes the line only when it is written, after every hook — so sensitive data never enters the audit chain.
