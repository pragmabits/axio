...
## Coverage
Every value a line carries: the message; the entry's error, by its message; strings, errors and fmt.Stringer annotations, by their text; []byte, by the text it holds (bytes that are not UTF-8 text become [REDACTED]); any string with the shape of base64 (standard or URL alphabet, padded or not) — message, error, annotation, nested value, a struct's []byte field — is also decoded and masked when its text carries PII; a JWT payload anywhere in a text is decoded and masked as JSON, sensitive claims included; and structured values — maps, slices, structs, pointers, http.Header — walked as their JSON encoding, keys checked against the sensitive fields and strings against the patterns at every level. Annotable values such as HTTP are expanded into their fields before any hook runs. A structured value that needed masking is written as its masked JSON tree (object keys in alphabetical order). A container nested deeper than the depth limit (default 32) becomes [REDACTED] whole; set it with WithPIIMaxDepth(n), piiMaxDepth in the config file, or PIIConfig.MaxDepth.
